...
#define SSID "sjr_family2.4G"  //<<<<<<   WiFi name
#define PASSWORD "0624368558"  //<<<<<<  PASSWORD

#define LINE_TOKEN "A0rz3NA38Dvx4JAlfEbZNyg3qRnVhnYNx54jwBZcTFw"  //<<<<<< Line Token Here
...
void loop() {
  ...
  if (digitalRead(Take_Pic) == 1 && startTimer != true) {
    Camera_capture();
    Serial.println("OK");
    startTimer = true;
  } else if (digitalRead(Take_Pic) == 0) {
    startTimer = false;
    time_capture = 0;
  }
  ...
  Serial.println(digitalRead(Take_Pic));
  delay(200);
}
...
void Send_line(uint8_t *image_data, size_t image_size) {
  LINE.notifyPicture("ภาพถ่ายปัจจุบัน", image_data, image_size);
}